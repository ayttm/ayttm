...
{
	gint service_id =
		get_service_id(gtk_combo_box_get_active_text(GTK_COMBO_BOX
			(service_list)));
	...
	my_contact =
		find_contact_in_group_by_nick(gtk_entry_get_text(GTK_ENTRY
			(nick)), g);
...
void edit_contact_window_new(struct contact *c)
{
	...
	if (!window_open) {
		...
		for (gwalker = list; gwalker; gwalker = g_list_next(gwalker)) {
			gtk_combo_box_append_text(GTK_COMBO_BOX(group_list),
				(gchar *)gwalker->data);
		}
...

		{
			...
			for (i = 0, l = l2; l; i++, l = g_list_next(l)) {
				...
				if (!strcmp(eb_services[c->default_chatb].name,
						label))
					def = i;
			}
			g_list_free(l2);
		...
		edit_contact_window = gtk_dialog_new_with_buttons("Ayttm",
		...
		gtk_dialog_set_default_response(GTK_DIALOG(edit_contact_window),
			GTK_RESPONSE_ACCEPT);
	...
	if (GTK_RESPONSE_ACCEPT ==
		gtk_dialog_run(GTK_DIALOG(edit_contact_window)))
		ok_callback(c);
