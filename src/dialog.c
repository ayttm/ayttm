	...
	void (*action) (const char *text, void *data), void *data)
{
	...
	gtk_box_pack_start(GTK_BOX(dialog_content_area), label, FALSE, FALSE,
		10);
	...
	if (result == GTK_RESPONSE_ACCEPT) {
		...
		gtk_tree_model_get(GTK_TREE_MODEL(list_store), &iter, 0, &chars,
			-1);
		action(chars, data);
	}
...
void eb_do_dialog(const char *message, const char *title,
	void (*action) (void *, int), void *data)
{
...
int eb_do_confirm_dialog(const char *message, const char *title)
{
	...
	gtk_box_pack_start(GTK_BOX(dialog_content_area), content_area, TRUE,
		TRUE, 2);
	...
	gtk_box_pack_start(GTK_BOX(content_area),
		gtk_image_new_from_stock(GTK_STOCK_DIALOG_QUESTION,
			GTK_ICON_SIZE_DIALOG), FALSE, FALSE, 5);
	...
	void *data)
{
	...
	GtkWidget *dialog = gtk_dialog_new_with_buttons(_("Ayttm"),
	...
	gtk_editable_insert_text(GTK_EDITABLE(text), value, strlen(value),
		&dummy);
